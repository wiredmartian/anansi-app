...
export class LevelTwoPage {
  ...
  onValueChange(event, form: NgForm, model: NgModel){
    ...
    /* Diagonally */
    this.calculateDiaOneSum();
    this.calculateDiaTwoSum();
    ...
    if(this.isLevelCompleted){
      this.clearLevelOneCompletion();
    }
  }

/* Diagonal values */

  isDiaOneValid() : boolean{
  ...
  calculateDiaOneSum() : number{
    this.blockSum.diaOneSum = 0
    if(this.isDiaOneValid()){
      this.blockSum.diaOneSum = parseInt(this.rowOneValues.row1NumberOne.toString())
      + parseInt(this.rowTwoValues.row2NumberTwo.toString())
      + parseInt(this.rowThrValues.row3NumberThree.toString());
    }
    return this.blockSum.diaOneSum;
  }

  calculateDiaTwoSum() : number{
    this.blockSum.diaTwoSum = 0;
    if(this.isDiaTwoValid()){
      this.blockSum.diaTwoSum = parseInt(this.rowOneValues.row1NumberThree.toString())
      + parseInt(this.rowTwoValues.row2NumberTwo.toString())
      + parseInt(this.rowThrValues.row3NumberOne.toString());
    }
    return this.blockSum.diaTwoSum;
  }
...
calculateRowThrSum() : number{
  ...
  isLevelCleared() : boolean{
    ...
    if (this.calculateRowOneSum() == 15 && this.calculateRowTwoSum() == 15 && this.calculateRowThrSum() == 15
          ...
          && this.calculateDiaOneSum() + this.calculateDiaTwoSum() == 30){
